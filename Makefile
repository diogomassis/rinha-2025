...
rinha: $@ ## Generate Pbs and build for rinha
...
$(project):
	...
	go build -o ${BIN_DIR}/${SERVER_BIN} .

test: all ## Launch tests
	go test ./...

run: all ## Run the server
	@echo "Starting gRPC server on :8000 and HTTP gateway on :8081"
	./${BIN_DIR}/${SERVER_BIN}

clean: clean_rinha ## Clean generated files
	${RM_F_CMD} ssl/*.crt
	${RM_F_CMD} ssl/*.csr
	${RM_F_CMD} ssl/*.key
	${RM_F_CMD} ssl/*.pem
	${RM_RF_CMD} ${BIN_DIR}
	${RM_RF_CMD} ${THIRD_PARTY_DIR}
